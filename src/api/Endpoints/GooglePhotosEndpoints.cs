...
public static class GooglePhotosEndpoints
{
    ...
    private static IResult GoogleOAuthRedirect(ILogger<GooglePhotosFlow> logger, HttpContext context, IConfiguration config, IMemoryCache sessionCache)
    {
        ...
        GoogleOAuthRedirectResponse response = new()
        {
    ...
    private static async Task<IResult> HandleOAuthResponse(HttpContext context, LinkSessions linkSessions, GooglePhotosFlow googlePhotos, ILogger<GooglePhotosFlow> logger)
    {
        ...
        if (StringValues.IsNullOrEmpty(returnedStateKey))
            return GlobalHelpers.CreateErrorPage(context, "There was a problem retrieving the google authorization code.");
        ...
        if (storedStateKey != returnedStateKey)
        {
            ...
            return GlobalHelpers.CreateErrorPage(context, "An error occurred when trying to link your account.", "Please ensure cookies are enabled and try again.");
        }

        if (context.Request.Query.ContainsKey("error"))
            return GlobalHelpers.CreateErrorPage(context, "There was a problem allowing <strong>Lightsaver</strong> to access your photos.");
        ...
        if (linkSession is null)
        {
            ...
            return GlobalHelpers.CreateErrorPage(context, "There was a problem linking your google account to lightsaver.");
        }
        ...
        if (!linkSessions.LinkAccessToken(accessTokenJson.AccessToken, sessionId))
        {
            ...
            linkSessions.ExpireSession(sessionId);
            return GlobalHelpers.CreateErrorPage(context, "LightSaver failed to link to google.", "Please Try Again");
        }
        ...
        catch
        {
            ...
            linkSessions.ExpireSession(sessionId);
            return GlobalHelpers.CreateErrorPage(context, "LightSaver is unable to connect to Google Photos.");
        }
